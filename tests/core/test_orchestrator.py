    ...
    async def generate(self, messages, config):
        ...
        if any("score each dimension" in m.content.lower() for m in messages):
            return json.dumps({
                ...
                "strengths": ["Good structure"], "improvements": ["Add reliability"],
            })
        elif any("debrief" in m.content.lower() for m in messages):
            return json.dumps({
                "overall_score": 3.5, "dimension_scores": {"structure": 4, "tradeoffs": 3},
...
class TestTurnOrchestrator:
    ...
    async def test_generate_debrief_emits_event(self) -> None:
        ...
        orchestrator = TurnOrchestrator(
            session=session, backend=backend, scorer=scorer, bus=bus,
            config=ModelConfig(model="test"), must_cover=[],
        )
